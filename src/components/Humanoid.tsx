...
export function Humanoid({ position = [0, 0, 0], color, isMoving, isShooting, scale = 1, hasGun = false }: HumanoidProps) {
  ...
  const suitMaterial = useMemo(() => new MeshStandardMaterial({ color, roughness: 0.45, metalness: 0.15 }), [color]);
  const skinMaterial = useMemo(() => new MeshStandardMaterial({ color: '#f5ccbe', roughness: 0.75 }), []);
  const shoeMaterial = useMemo(() => new MeshStandardMaterial({ color: '#0d0d0d', roughness: 0.2, metalness: 0.4 }), []);
  const shirtMaterial = useMemo(() => new MeshStandardMaterial({ color: '#ffffff', roughness: 0.9 }), []);
  const goldMaterial = useMemo(() => new MeshStandardMaterial({ color: '#ffcc00', roughness: 0.1, metalness: 0.95, emissive: '#aa7a00', emissiveIntensity: 0.3 }), []);
  ...
  useFrame((state) => {
    ...
    if (isMoving && !isShooting) {
      ...
      if (bodyRef.current) {
        bodyRef.current.position.y = MathUtils.lerp(bodyRef.current.position.y, 0.95, 0.1);
        bodyRef.current.rotation.y = MathUtils.lerp(bodyRef.current.rotation.y, 0, 0.1);
        bodyRef.current.rotation.z = MathUtils.lerp(bodyRef.current.rotation.z, 0, 0.1);
      }
      ...
      if (leftKneeRef.current) leftKneeRef.current.rotation.x = MathUtils.lerp(leftKneeRef.current.rotation.x, 0, 0.2);
      if (rightKneeRef.current) rightKneeRef.current.rotation.x = MathUtils.lerp(rightKneeRef.current.rotation.x, 0, 0.2);
    ...
    if (isShooting) {
      if (bodyRef.current) {
        // Twist slightly
        bodyRef.current.rotation.y = MathUtils.lerp(bodyRef.current.rotation.y, -Math.PI / 6, 0.3);
      }
      ...
      if (hasGun) {
        ...
        // Punch-like behavior
        if (rightArmRef.current) {
          rightArmRef.current.rotation.x = MathUtils.lerp(rightArmRef.current.rotation.x, -Math.PI / 2 + 0.2, 0.3);
          rightArmRef.current.rotation.z = MathUtils.lerp(rightArmRef.current.rotation.z, -0.2, 0.3);
        }
        if (rightElbowRef.current) rightElbowRef.current.rotation.x = MathUtils.lerp(rightElbowRef.current.rotation.x, 0, 0.3);
      }
      
      // Guard left hand
      if (leftArmRef.current) {
        leftArmRef.current.rotation.x = MathUtils.lerp(leftArmRef.current.rotation.x, -Math.PI / 3, 0.2);
      ...
      // Clean up rotation z
      if (rightArmRef.current) rightArmRef.current.rotation.z = MathUtils.lerp(rightArmRef.current.rotation.z, 0.1, 0.1);
      if (leftArmRef.current) leftArmRef.current.rotation.z = MathUtils.lerp(leftArmRef.current.rotation.z, -0.1, 0.1);
    }
  });
    ...
    <group ref={rootRef} position={position} scale={scale}>
      ...
      <group ref={bodyRef} position={[0, 0.95, 0]}>
        ...
        <group position={[0, 0.3, 0]}>
          ...
          {/* Tie */}
          <mesh position={[0, 0.04, 0.13]} material={new MeshStandardMaterial({ color: color === '#000000' ? '#1a1a1a' : '#444444', roughness: 0.8 })}>
        ...
        <group ref={headRef} position={[0, 0.72, 0]}>
          ...
          <group position={[0, 0.05, -0.04]}>
            <mesh material={new MeshStandardMaterial({ color: '#111111', roughness: 0.9 })}>
              <boxGeometry args={[0.23, 0.22, 0.18]} />
            </mesh>
            <mesh position={[0, 0.12, 0.03]} material={new MeshStandardMaterial({ color: '#111111', roughness: 0.9 })}>
          ...
          <group position={[0, 0.04, 0.11]}>
            <mesh material={new MeshStandardMaterial({ color: '#050505', roughness: 0.05, metalness: 0.9 })}>
              <boxGeometry args={[0.21, 0.05, 0.035]} />
            </mesh>
            {/* Sunglasses temporal temples lines */}
            <mesh position={[-0.105, 0.015, -0.05]} material={new MeshStandardMaterial({ color: '#050505', roughness: 0.1 })}>
          ...
          <group ref={rightElbowRef} position={[0, -0.32, 0]}>
            ...
            {hasGun && (
              <group position={[0, -0.42, 0.06]} rotation={[Math.PI / 2, 0, 0]}>
                {/* Barrel / slide */}
                <mesh castShadow material={new MeshStandardMaterial({ color: '#1a1a1a', roughness: 0.15, metalness: 0.85 })}>
                  <boxGeometry args={[0.04, 0.06, 0.18]} />
                </mesh>
                {/* Grip / handle */}
                <mesh position={[0, -0.05, -0.04]} rotation={[-0.3, 0, 0]} material={new MeshStandardMaterial({ color: '#111111', roughness: 0.7 })}>
                  <boxGeometry args={[0.035, 0.08, 0.035]} />
                </mesh>
                {/* Slide steel highlights */}
                <mesh position={[0, 0.025, 0.04]} material={new MeshStandardMaterial({ color: '#ffffff', roughness: 0.2, metalness: 0.9 })}>
        ...
        <group position={[0, -0.1, 0]}>
          ...
          <group ref={leftLegRef} position={[-0.12, 0, 0]}>
            ...
            <group ref={leftKneeRef} position={[0, -0.42, 0]}>
              ...
              <mesh castShadow receiveShadow position={[0, -0.48, 0.03]} material={shoeMaterial}>
